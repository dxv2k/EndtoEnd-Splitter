...
function check_valid($file_size, $file_type)
{
    ...
    if(!in_array($file_type, ["mp3", "mp4", "wav", "mpeg"])) {
...
if (isset($_POST['submit']) && $_POST['submit'] == 'upload')
{
    ...
    {        
        ...
        check_valid($_FILES["audio_in"]["size"], $imageFileType);
        ...
        } else 
        {
            if (!file_exists($uploadFilePath)) // check if file already uploaded in uploaded folder
            {
                if (move_uploaded_file($_FILES["audio_in"]["tmp_name"], $uploadFilePath)) {
                    ...
                    {
                        $err_code = null;
                        system("python st.py $stems_option $uploadFilePath $save_path ", $err_code); // if want to catch output msg add 2>&1 to tail
                        ...
                        if($conn->query("INSERT INTO processed_folders (folder_name, parent_file_id, user_id, stem_option, uploaded_on, download) VALUES ('$folder_name', $parent, '$stems_option', $cur, NOW(), '$tmp2')"))
                        {
